...
# Compare classification performance of existing methods
## Ensemble enet
...
y_test=Y.test
# t0 <- proc.time()
# enetConcat <- enetConcat(X_train, X_test, y_train=Y.train, y_test=Y.test, penalties, folds, iter)
# t1 <- proc.time()
# (t1-t0)/60 # 4.9023333  min
# saveRDS(enetConcat, "~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/enetConcat.rds")
enetConcat <- readRDS("~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/enetConcat.rds")

# sPLSDA concat
comps <- nlevels(Y.train)-1
keepX <- rep(round(sum(enetConcat$panelLength$panelLength)/comps, 0), comps)
names(keepX) <- enetConcat$panelLength$dataset
keepX <- keepX[names(X_train)]
# t0 <- proc.time()
# splsdaConcat <- splsdaConcat(X_train, X_test, y_train, y_test, keepX=keepX[names(X.test)], ncomp=comps, folds, iter)
# t1 <- proc.time()
# (t1-t0)/60 # 3.5210333 min
# saveRDS(splsdaConcat, "~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/splsdaConcat.rds")
splsdaConcat <- readRDS("~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/splsdaConcat.rds")

# DIABLO - concat
# t0 <- proc.time()
# diabloMods_concat <- diabloFit(X_train=X.train, X_test=X.test, y_train=Y.train, y_test=Y.test, selectVar=splsdaConcat$panelLength, ncomp = comps, folds=folds, iter=iter)
# t1 <- proc.time()
# (t1-t0)/60 # 17.963767 min
# saveRDS(diabloMods_concat, "~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/diabloMods_concat.rds")
diabloMods_concat<- readRDS("~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/diabloMods_concat.rds")


# Enet Ensemble
# t0 <- proc.time()
# enetEnsemble <- enetEnsemble(X_train=X.train[names(X.test)], X_test=X.test, y_train=Y.train, y_test=Y.test, penalties, folds, iter)
# t1 <- proc.time()
# (t1-t0)/60 # 17.7478167  min
# saveRDS(enetEnsemble, "~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/enetEnsemble.rds")
enetEnsemble <- readRDS("~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/enetEnsemble.rds")
...
iter = 5
ncomp = 3
keepX <- lapply(enetEnsemble$panelLength$panelLength, function(i){
  rep(round(i/ncomp, 0), ncomp)
})
names(keepX) <- enetEnsemble$panelLength$dataset
keepX <- keepX[names(X_train)]
# t0 <- proc.time()
# splsdaEnsem <- splsdaEnsemble(X_train, X_test, y_train, y_test, keepX, ncomp, k, iter)
# t1 <- proc.time()
# (t1-t0)/60 # 1.2538833 min
saveRDS(splsdaEnsem, "~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/splsdaEnsem.rds")
splsdaEnsem <- readRDS("~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/splsdaEnsem.rds")

# DIABLO - ensemble
# t0 <- proc.time()
# diabloMods_ensem <- diabloFit(X_train=X.train, X_test=X.test, y_train=Y.train, y_test=Y.test, selectVar=splsdaEnsem$panelLength, ncomp = comps, folds=folds, iter=iter)
# t1 <- proc.time()
# (t1-t0)/60 # 19.69945  min
# saveRDS(diabloMods_ensem, "~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/diabloMods_ensem.rds")
diabloMods_ensem <- readRDS("~/Dropbox/PROOF/Manuscript/mixOmics/diablo_datasets/brca/diabloMods_ensem.rds")



minMaxError <- rbind(enetConcat$error, splsdaConcat$error,
  diabloMods_concat[, colnames(enetConcat$error)],
  enetEnsemble$error, splsdaEnsem$error,
  diabloMods_ensem[, colnames(enetEnsemble$error)]) %>% 
  filter(cohort == "test") %>% 
  filter(error == min(error) | error == max(error))

## plot error rates
concat <- rbind(enetConcat$error, splsdaConcat$error,
  diabloMods_concat[, colnames(enetConcat$error)]) %>% 
  mutate(var = paste(paste(enetConcat$panelLength$panelLength, enetConcat$panelLength$dataset), collapse = ",")) %>% 
  ggplot(aes(x = method, y = error, fill = cohort, color=cohort)) +
  geom_hline(yintercept = c(minMaxError$error), linetype = "dashed") +
  geom_point() +
  geom_errorbar(aes(ymin=error-sd, ymax=error+sd)) +
  facet_wrap(~var) +
  customTheme(sizeStripFont = 15, xAngle = 60, hjust = 1, vjust = 1, xSize = 10, ySize = 10, xAxisSize = 10, yAxisSize = 10) +
  ylab("Balanced Error Rate")

ensem <- rbind(enetEnsemble$error, splsdaEnsem$error,
  diabloMods_ensem[, colnames(enetEnsemble$error)]) %>% 
  mutate(var = paste(paste(enetEnsemble$panelLength$panelLength, enetEnsemble$panelLength$dataset), collapse = ",")) %>%
  ggplot(aes(x = method, y = error, fill = cohort, color=cohort))+
    geom_hline(yintercept = c(minMaxError$error), linetype = "dashed") +
  geom_point() +
  geom_errorbar(aes(ymin=error-sd, ymax=error+sd)) +
...
plot_grid(concat, ensem, labels = c("A.", "B."))
