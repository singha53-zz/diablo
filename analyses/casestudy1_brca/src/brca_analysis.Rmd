...
links <- data.frame(from = rep(names(diabloPanel), sapply(diabloPanel, length)),
                    to = unlist(diabloPanel))
nodes <- data.frame(id = c(names(diabloPanel), unique(unlist(diabloPanel))))
...
```{r}
## mRNA
featplot_mrna <- rbind(selectVar(diablo, comp = 1)[["mRNA"]][["value"]], 
  selectVar(diablo, comp = 2)[["mRNA"]][["value"]],
  selectVar(diablo, comp = 3)[["mRNA"]][["value"]]) %>% 
  data.frame() %>% 
  mutate(dataset = "mRNA",
         feature = rownames(.),
  ...
  ggplot(aes(x = feature, y = value.var)) +
  geom_bar(stat = "identity") +
  ggtitle("mRNA") +
  ylab("Variable contribution (absolute loading value)") +
...
## miRNA
featplot_mirna <- lapply(selectVar(diablo)[1:4], function(i){i$value}) %>% 
  do.call(rbind, .) %>% 
  data.frame() %>% 
...
featplot_mirna
## CpGs
featplot_cpgs <- lapply(selectVar(diablo)[1:4], function(i){i$value}) %>% 
  do.call(rbind, .) %>% 
  data.frame() %>% 
  mutate(dataset = sapply(strsplit(rownames(.), "\\."), function(i) i[1]),
         feature = sapply(strsplit(rownames(.), "\\."), function(i) i[2]),
...
featplot_cpgs
## Proteins
featplot_proteins <- lapply(selectVar(diablo)[1:4], function(i){i$value}) %>% 
  do.call(rbind, .) %>% 
  data.frame() %>% 
  mutate(dataset = sapply(strsplit(rownames(.), "\\."), function(i) i[1]),
         feature = sapply(strsplit(rownames(.), "\\."), function(i) i[2]),
...
plot_grid(featplot_mrna, featplot_mirna, featplot_cpgs, featplot_proteins, nrow = 2)
