...
#cv <- perf(diablo, validation = "Mfold", folds = 5, nrepeat = 5, cpus = 5)
#cv$WeightedVote.error.rate
...
```{r}
comp1 <- lapply(diablo$variates, function(i){
  i[, 1]
}) %>% do.call(cbind, .) %>% 
  cor()
comp2 <- lapply(diablo$variates, function(i){
  i[, 2]
}) %>% do.call(cbind, .) %>% 
  cor()
comp3 <- lapply(diablo$variates, function(i){
  i[, 3]
}) %>% do.call(cbind, .) %>% 
  cor()

comp1.3 <- matrix(NA, nrow(comp1), ncol(comp1))
colnames(comp1.3) <- rownames(comp1.3) <- colnames(comp1)
for(i in 1 : nrow(comp1)){
  for(j in 1 : ncol(comp1)){
    comp1.3[i, j] <- paste(round(c(comp1[i,j], comp2[i,j], comp3[i,j]), 2), collapse = ",")
  }
}
comp1.3
```
...
enrichment <- enrichment0 %>% 
  ...
  dplyr::select(collection, mrna:protein)
  
n_sigAll0 <- enrichment %>% 
  group_by(collection) %>% 
  dplyr::summarise(n = n())
n_sigAll <- as.matrix(n_sigAll0[-1, -1])
rownames(n_sigAll) <- n_sigAll0$collection[-1]
...
n_sigSingle0 <- enrichment %>% 
  ...
  dplyr::summarise(mrna = sum(mrna),
                   cpg = sum(cpg),
                   protein = sum(protein))
n_sigSingle <- as.matrix(n_sigSingle0[-1, -1])
rownames(n_sigSingle) <- n_sigSingle0$collection[-1]
